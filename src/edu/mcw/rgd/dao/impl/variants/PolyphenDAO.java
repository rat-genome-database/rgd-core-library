...
public class PolyphenDAO extends AbstractDAO {
    public List<PolyPhenPrediction> getPloyphenDataByVariantId(int variantId, int transcriptId) throws Exception{
        String sql="select * from polyphen where variant_rgd_id=? and transcript_rgd_id=?";
        PolyPhenMapper query=new PolyPhenMapper(DataSourceFactory.getInstance().getCarpeNovoDataSource(), sql);
        return execute(query, variantId, transcriptId);
    }
}
