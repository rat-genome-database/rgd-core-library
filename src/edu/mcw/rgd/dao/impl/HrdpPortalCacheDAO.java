...
public class HrdpPortalCacheDAO extends AbstractDAO {
    ...
    public int updateHrdpPortalData(HrdpPortalCache hrdp) throws Exception{
        String sql= """
                ...
                has_child_pheno_count=?,has_parent_sample_count=?,has_child_sample_count=?,has_phenominer=?,has_variant_visualizer=?
                Where strain_id=? and group_name=?
                """;
        return update(sql,hrdp.getStrainSymbol(),hrdp.getParentOntId(),hrdp.getChildOntIds(),hrdp.getParentSampleIds(),hrdp.getChildSampleIds(),hrdp.getHasParentPhenoCount(),hrdp.getHasChildPhenoCount(),hrdp.getHasParentSampleCount(),hrdp.getHasChildSampleCount(),hrdp.getHasPhenominer(),hrdp.getHasVariantVisualizer(),hrdp.getStrainId(),hrdp.getGroupName());
    }
