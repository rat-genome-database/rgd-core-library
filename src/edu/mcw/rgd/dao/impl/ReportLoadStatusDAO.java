...
public class ReportLoadStatusDAO extends AbstractDAO {
    ...
    /** Returns the most recently updated record with pending/processing/failed status, or null if no active/incomplete batch. */
    public ReportLoadStatus getActiveBatchInfo() throws Exception {
        String sql = "SELECT * FROM report_load_status WHERE status IN ('pending', 'processing', 'failed') ORDER BY updated_at DESC LIMIT 1";
        ReportLoadStatusQuery q = new ReportLoadStatusQuery(this.getDataSource(), sql);
