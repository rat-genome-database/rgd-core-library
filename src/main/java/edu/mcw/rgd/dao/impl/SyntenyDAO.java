...
public class SyntenyDAO extends AbstractDAO {
    ...
    public List<SyntenicRegion> getBlocksInRange(int backboneMapKey, String chromosome, int start, int stop, int backboneBlockMinSize, int backboneBlockMaxSize, int mapKey) throws Exception {
        String query = "SELECT * FROM synteny_ucsc WHERE map_key1=? AND map_key2=? " +
                " AND start_pos1<? AND stop_pos1 > ? AND chromosome1=? AND pos_len1 BETWEEN ? AND ? " +
                "ORDER BY chromosome1, start_pos1";
        SyntenyQuery q = new SyntenyQuery(this.getDataSource(), query);
        return execute(q, backboneMapKey, mapKey, stop, start, chromosome, backboneBlockMinSize, backboneBlockMaxSize);
    }
    ...
    public List<SyntenicRegion> getGapsInRange(int backboneMapKey, String chromosome, int start, int stop, int backboneGapMinSize, int backboneGapMaxSize, int mapKey) throws Exception {
        String query = "SELECT * FROM synteny_ucsc_gaps WHERE map_key1=? AND map_key2=? " +
                " AND start_pos1<? AND stop_pos1 > ? AND chromosome1=? AND pos_len1 BETWEEN ? AND ? " +
                "ORDER BY chromosome1, start_pos1";
        SyntenyQuery q = new SyntenyQuery(this.getDataSource(), query);
        return execute(q, backboneMapKey, mapKey, stop, start, chromosome, backboneGapMinSize, backboneGapMaxSize);
    }
